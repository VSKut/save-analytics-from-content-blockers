...
export async function init () {
    ...
    try {
        ...
        app.use("api/api/robots.txt", (_, res) => res.status(200).set("Content-Type", "text/plain").send(
            'User-agent: *\nDisallow: /'
        ));
        ...
        if (config.isLocalEnv) {
            app.use("api/api/", express.static(`${ __dirname }/../static-test`));
        } else {
            app.use("api/api/", (_, res) => res.status(200).set("Content-Type", "text/html").send(
